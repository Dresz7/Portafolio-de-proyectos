...
import { styles } from "./QrScannedOutScreen.style";

export function QrScannedOutScreen({ route }) {
  ...
  const handleConfirmar = async () => {
    try {
      ...
      const response = await fetch(
        "http://72.167.55.139:8080/ExpoApi/Register/RegisterOut/UpdateRegister.php",
        {
          method: "POST",
          body: formData,
        }
        ...
        Alert.alert("Confirmación", responseData.success, [
          {
            ...
            onPress: () => {
              navigation.navigate(screen.registerOut.tab, {
                screen: screen.registerOut.registerOut,
              });
            },
          },
        ]);
