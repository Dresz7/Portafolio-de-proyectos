...
(function ($) {
  $.fn.tableExport = function (options) {
    ...
    if (defaults.date.html.length) {
      ...
      f =
        (formatItems.indexOf("yyyy") >= 0
          ? formatItems.indexOf("yyyy")
          : formatItems.indexOf("yy")) + 1;
    ...
    if (
      ...
      rowLength += CollectCsvData(
        $(el).find("thead").first().find(defaults.theadSelector),
        "th,td",
        rowLength
      );
      ...
      if (defaults.tfootSelector.length)
        CollectCsvData(
          $(el).find("tfoot").first().find(defaults.tfootSelector),
          "td,th",
          rowLength
        );
        ...
        ForEachVisibleCell(
          ...
          function (cell, row, col) {
            ...
            if (colName.indexOf(defaults.sql.columnEnclosure) > -1)
              colName = replaceAll(
                colName.toString(),
                defaults.sql.columnEnclosure,
                defaults.sql.columnEnclosure + defaults.sql.columnEnclosure
              );
            tdData +=
              defaults.sql.columnEnclosure +
              colName +
      ...
      $(el)
        ...
        .each(function () {
          ...
          const rowLength = CollectXmlssData(
      ...
      for (let n = 0, c = docNames.length; n < c; n++) {
        ...
        if (itemCount === 2)
          docNames[firstOccurrences[item]] =
            docNames[firstOccurrences[item]].substring(0, 29) + "-1";
        if (count[item] > 1)
      ...
      $(el)
        ...
        .each(function () {
          ...
          if (
            ...
            sheetName =
              defaults.mso.worksheetName + " " + (sheetNames.length + 1);
          else if (
            typeof defaults.mso.worksheetName[sheetNames.length] !== "undefined"
          )
      ...
      saveToFile(
        xlsxWorkbookToArrayBuffer(wbData),
              ...
              function (cell, row, col) {
                if (cell !== null) {
                  ...
                  if (defaults.mso.styles.length) {
                    const cellStyles = document.defaultView.getComputedStyle(
                      cell,
                      null
                    );
                    const rowStyles = document.defaultView.getComputedStyle(
                      $row[0],
                      null
                    );
      ...
      if (defaults.pdfmake.enabled === true) {
              ...
              layout: {
                ...
                hLineColor: function (i, node) {
                  return i < node.table.headerRows
                    ...
                    : defaults.pdfmake.docDefinition.styles.alternateRow
                        .fillColor;
                },
        ...
        if (
          ...
          typeof pdfMake.createPdf !== "undefined"
        ) {
          pdfMake.fonts = {
            Roboto: {
        ...
        doc.html(el[0], {
          ...
          html2canvas: {
            scale:
              (doc.internal.pageSize.width - defaults.jspdf.margins.left * 2) /
              el[0].scrollWidth,
          },
          ...
          $(el)
            ...
            .each(function () {
                ...
                atOptions.drawCell = function (cell, data) {
                  ...
                  if (draw2canvas !== true) {
                    if (prepareAutoTableText(cell, data, teCell)) {
                      ...
                      if (
                        typeof teCell !== "undefined" &&
                        (typeof teCell.hasUserDefText === "undefined" ||
                          teCell.hasUserDefText !== true) &&
              ...
              defaults.jspdf.autotable.startY =
                jsPdfAutoTableEndPosY() + atOptions.margin.top;
            });
    ...
    function findTableElements($parent, selector) {
      ...
      return $parent.find(selector).filter(function () {
        return (
          parentLevel ===
          $(this).closest(parentSelector).parents(parentSelector).length
        );
      });
    }
    ...
    function GetColumnNames(table) {
      ...
      $(table)
        ...
        .each(function (index, el) {
          ...
          if (
            typeof el.parentNode.rowIndex !== "undefined" &&
    ...
    function isVisible($element) {
      ...
      const isCell =
        isRow === false && typeof $element[0].cellIndex !== "undefined";
    ...
    function drawAutotableElements(cell, elements, teOptions) {
      elements.each(function () {
        if ($(this).is("div")) {
          const bColor = rgb2array(
            getStyle(this, "background-color"),
            [255, 255, 255]
          );
          const lColor = rgb2array(
            getStyle(this, "border-top-color"),
            [0, 0, 0]
          );
          const lWidth = getPropertyUnitValue(
            this,
            "border-top-width",
            defaults.jspdf.unit
          );
    ...
    function getCellStyles(cell) {
      ...
      const result = {
        style: {
          align: a,
          bcolor: rgb2array(
            getStyle(cell, "background-color"),
            [255, 255, 255]
          ),
    ...
    function xlsxTableToSheet(table) {
      ...
      for (; _R < rows.length && R < sheetRows; ++_R) {
        ...
        for (_C = C = 0; _C < elts.length; ++_C) {
          ...
          let _t = "";
          const cellFormat =
            $(elt).attr("data-tableexport-cellformat") || undefined;
          ...
          if (v != null) {
            ...
            if (v.length === 0) {
              ...
              _t === "n" ||
              isFinite(xlsxToNumber(v, defaults.numbers.output))
            ) {
                ...
                o = {
                  ...
                  z:
                    typeof ssfId === "string"
                      ...
                      : ssfId in ssfTable
                      ? ssfTable[ssfId]
                      : ssfId === defaults.mso.xlsx.formatId.currency
                      ? defaults.mso.xlsx.format.currency
                      : "0.00",
                };
            ...
            if ($aTag && $aTag.length) {
              const href = $aTag[0].hasAttribute("href")
                ? $aTag.attr("href")
                : "";
              const content =
                defaults.htmlHyperlink !== "href" || href === "" ? v : "";
              const hyperlink =
                href !== ""
                  ? '=HYPERLINK("' +
                    href +
              ...
              if (hyperlink !== "") {
                if (typeof defaults.mso.xlsx.onHyperlink === "function") {
                  v = defaults.mso.xlsx.onHyperlink(
                    $(elt),
                    _R,
                    _C,
                    href,
    ...
    function xlsxToNumber(s, numbersFormat) {
      ...
      if ("" !== numbersFormat.thousandsSeparator)
        ss = ss.replace(
          new RegExp(
            "([\\d])" + numbersFormat.thousandsSeparator + "([\\d])",
            "g"
          ),
          "$1$2"
        );
      if ("." !== numbersFormat.decimalMark)
        ss = ss.replace(
          new RegExp("([\\d])" + numbersFormat.decimalMark + "([\\d])", "g"),
          "$1.$2"
        );
    ...
    function saveToFile(data, fileName, type, charset, encoding, bom) {
      ...
      if (typeof defaults.onBeforeSaveToFile === "function") {
        saveIt = defaults.onBeforeSaveToFile(
          data,
          fileName,
          type,
          charset,
    ...
    function jsPdfAutoTable(doc, headers, data, options) {
      ...
      jsPdfCursor = {
        y:
          jsPdfSettings.startY === false
            ? jsPdfSettings.margin.top
            : jsPdfSettings.startY,
      };
      ...
      const firstRowHeight =
        jsPdfTable.rows[0] && jsPdfSettings.pageBreak === "auto"
          ? jsPdfTable.rows[0].height
          : 0;
      let minTableBottomPos =
        jsPdfSettings.startY +
        jsPdfSettings.margin.bottom +
        jsPdfTable.headerRow.height +
        firstRowHeight;
      if (jsPdfSettings.pageBreak === "avoid") {
        minTableBottomPos += jsPdfTable.height;
      }
      if (
        (jsPdfSettings.pageBreak === "always" &&
          jsPdfSettings.startY !== false) ||
        (jsPdfSettings.startY !== false &&
          minTableBottomPos > jsPdfDoc.internal.pageSize.height)
      ) {
        jsPdfDoc.addPage();
      ...
      if (
        jsPdfSettings.drawHeaderRow(
          jsPdfTable.headerRow,
    ...
    function jsPdfAutoTableEndPosY() {
      if (
        typeof jsPdfCursor === "undefined" ||
        typeof jsPdfCursor.y === "undefined"
      ) {
        return 0;
      }
    ...
    function jsPdfAutoTableText(text, x, y, styles) {
      ...
      const fontSize =
        jsPdfDoc.internal.getFontSize() / jsPdfDoc.internal.scaleFactor;
      ...
      if (styles.halign === "center" || styles.halign === "right") {
        ...
        if (splittedText && lineCount >= 1) {
          for (let iLine = 0; iLine < splittedText.length; iLine++) {
            jsPdfDoc.text(
              splittedText[iLine],
              x - jsPdfDoc.getStringUnitWidth(splittedText[iLine]) * alignSize,
              y
            );
            y += fontSize;
          }
      ...
      ["top", "right", "bottom", "left"].forEach(function (side, i) {
        if (typeof marginSetting === "number") {
          ...
          settings.margin[side] =
            typeof marginSetting[key] === "number" ? marginSetting[key] : 40;
        }
      });
    ...
    function jsPdfCreateModels(inputHeaders, inputData) {
      ...
      const themeStyles = jsPdfExtend(
        jsPdfDefaultStyles,
        jsPdfThemes[jsPdfSettings.theme].table,
        jsPdfThemes[jsPdfSettings.theme].header
      );
      headerRow.styles = jsPdfExtend(
        themeStyles,
        jsPdfSettings.styles,
        jsPdfSettings.headerStyles
      );
      ...
      inputHeaders.forEach(function (rawColumn, dataKey) {
        if (typeof rawColumn === "object") {
          dataKey =
            typeof rawColumn.dataKey !== "undefined"
              ? rawColumn.dataKey
              : rawColumn.key;
        }
        ...
        cell.contentWidth =
          ...
          jsPdfGetStringWidth(cell.text, cell.styles);
      ...
      inputData.forEach(function (rawRow, i) {
        ...
        const themeStyles = jsPdfExtend(
          jsPdfDefaultStyles,
          jsPdfThemes[jsPdfSettings.theme].table,
          isAlternate ? jsPdfThemes[jsPdfSettings.theme].alternateRow : {}
        );
        const userStyles = jsPdfExtend(
          jsPdfSettings.styles,
          jsPdfSettings.bodyStyles,
          isAlternate ? jsPdfSettings.alternateRowStyles : {}
        );
        ...
        jsPdfTable.columns.forEach(function (column) {
          ...
          jsPdfSettings.createdCell(
            cell,
            jsPdfHooksData({ column: column, row: row })
          );
          cell.contentWidth =
            cell.styles.cellPadding * 2 +
            jsPdfGetStringWidth(cell.text, cell.styles);
    ...
    function jsPdfCalculateWidths() {
      ...
      jsPdfTable.columns.forEach(function (column) {
        column.contentWidth =
          jsPdfTable.headerRow.cells[column.dataKey].contentWidth;
      ...
      const maxTableWidth =
        jsPdfDoc.internal.pageSize.width -
        jsPdfSettings.margin.left -
        jsPdfSettings.margin.right;
      ...
      jsPdfTable.width =
        preferredTableWidth < maxTableWidth
          ? preferredTableWidth
          : maxTableWidth;
      ...
      jsPdfTable.columns.forEach(function (column) {
        ...
        if (colStyles.columnWidth === "wrap") {
          ...
          if (
            column.contentWidth <= fairWidth &&
            jsPdfTable.contentWidth > jsPdfTable.width
          ) {
            column.width = column.contentWidth;
          } else {
      ...
      all.forEach(function (row, i) {
        ...
        row.height =
          row.heightStyle +
          lineBreakCount * row.styles.fontSize * FONT_ROW_RATIO +
    ...
    function jsPdfDistributeWidth(
      ...
      const extraWidth =
        jsPdfTable.width - staticWidth - dynamicColumnsContentWidth;
    ...
    function jsPdfPrintRows() {
      jsPdfTable.rows.forEach(function (row, i) {
        ...
        row.y = jsPdfCursor.y;
        if (
          jsPdfSettings.drawRow(row, jsPdfHooksData({ row: row })) !== false
        ) {
    ...
    function jsPdfIsNewPage(rowHeight) {
      const afterRowPos =
        jsPdfCursor.y + rowHeight + jsPdfSettings.margin.bottom;
